...
class BattleForcast():
    ...
    def draw(self, screen):
        ...
        pAttackText = font.render(str(max(0, currentUnit.attack - unitsInRange[attackUnitIndex].defense)), True, (0,0,0))
        pAttackRect = pAttackText.get_rect()
        pAttackRect.center = (self.X+75, self.Y+185) 

        pHitText = font.render("90", True, (0,0,0))
        pHitRect = pHitText.get_rect()
        pHitRect.center = (self.X+75, self.Y+330)

        pCritText = font.render("1", True, (0,0,0))
        ...
        eAttackText = font.render(str(max(0, unitsInRange[attackUnitIndex].attack - currentUnit.defense)), True, (0,0,0))
        eAttackRect = eAttackText.get_rect()
        eAttackRect.center = (self.X + 390, self.Y+185)

        eHitText = font.render("95", True, (0,0,0))
        eHitRect = eHitText.get_rect()
        eHitRect.center = (self.X+390, self.Y+330)

        eCritText = font.render("0", True, (0,0,0))
...
class Unit():
    ...
    def __init__(self, X, Y):
        ...
        self.Y = Y
        pics = []
        pics.append(pygame.transform.scale(protagPicA, (tileSize, tileSize)))
        pics.append(pygame.transform.scale(protagPicB, (tileSize, tileSize)))
        self.pics = pics
        self.aniTimer = 5
        self.active = True

    def draw(self, screen):
        screen.blit(self.pics[0], (self.X*tileSize, self.Y*tileSize))
        self.aniTimer -= 1
        if self.aniTimer < 0:
            tmpPic = self.pics.pop(0)
            self.pics.append(tmpPic)
            self.aniTimer = 5
...
## combat
attackingTimerStart = 10
attackingTimer = attackingTimerStart
                ...
                if selectingAttack:
                    ...
                    if event.type == pygame.KEYDOWN and event.key == pygame.K_RIGHT:
                        if attackUnitIndex < len(unitsInRange) - 1:
                            ...
                            attackUnitIndex = 0
                        mainCursor.X = unitsInRange[attackUnitIndex].X
                        mainCursor.Y = unitsInRange[attackUnitIndex].Y
